...
public class PathTest {//包含题目[3]
    ...
    public void testExtractLoopPath() {
       //TODO:[3]
        ...
        if (!fl || !oldPath.isValid()) {
            assertEquals(null,oldPath.extractLoopPath());
        } else {
            Path newPath = oldPath.extractLoopPath();
            assertNotEquals(null, newPath);
            boolean fl1 = false;
            for (int i = 0; i < oldNodes.size() - 1; i++) {
                boolean fl2 = false;
                for (int j = 0; j < newPath.size(); j++) {
                    if (i + j >= oldNodes.size() ||
                            !Objects.equals(oldNodes.get(i + j), newPath.getNode(j))
                            || newPath.getNode(0) != newPath.getNode(newPath.size() - 1)) {
                        fl2 = true;
                        break;
                    }
                }
